...
auto make_unique_linked_features_tuple()->std::unique_ptr<FeaturesTuple>
{
	auto features_ptr = std::make_unique<FeaturesTuple>();
	Akari::Graphics::Vulkan::Core::LinkFeaturesTuple<FeaturesTuple>(*features_ptr);
	return features_ptr;
}
int main(int argc, const char** argv)
{
	...
	auto instanceExtensionNames = list_traits::EnumerateRequiredVulkan13InstanceExtensionNames();
	{
		bool isFound = true;
	...
	auto deviceExtensionNames  = list_traits::EnumerateRequiredVulkan13DeviceExtensionNames();
	{
		bool isFound = true;
	...
	const float priority = 1.0f;
	{
		...
		for (auto& queueFamilyProp : queueFamilyProperties) {
			std::cout << queueFamilyProp.queueCount << std::endl;
		}
	}
	auto deviceQueueCreateInfos = std::vector<vk::DeviceQueueCreateInfo>{
		vk::DeviceQueueCreateInfo().setQueueFamilyIndex(0).setQueueCount(1).setPQueuePriorities(&priority),
		vk::DeviceQueueCreateInfo().setQueueFamilyIndex(1).setQueueCount(1).setPQueuePriorities(&priority),
		vk::DeviceQueueCreateInfo().setQueueFamilyIndex(2).setQueueCount(1).setPQueuePriorities(&priority),
	};
	...
	auto vkRaiiDevice = std::make_unique<vk::raii::Device>(
		(*vkRaiiPhysicalDevices)[0],
		deviceCreateInfo
	);
